...
func (worker *Worker) processNotification(ctx context.Context, id uuid.UUID) error {
	...
	// redelivered message is dropped without a duplicate send.
	err = worker.repo.UpdateStatus(ctx, id, domain.StatusProcessing, domain.StatusQueued, domain.StatusRetrying)
	if err != nil {
