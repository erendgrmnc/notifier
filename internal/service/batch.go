...
func (svc *NotificationService) CreateBatch(ctx context.Context, batchRepo BatchRepository, inputs []CreateInput) (BatchResult, error) {
	now := svc.clock.Now()
	batchID := uuid.New()
	...
	existingKeys, err := svc.existingKeysFor(ctx, batchRepo, inputs)
	if err != nil {
	...
	if err := batchRepo.CreateBatch(ctx, toInsert); err != nil {
...
func (svc *NotificationService) existingKeysFor(ctx context.Context, batchRepo BatchRepository, inputs []CreateInput) (map[string]uuid.UUID, error) {
	var keys []string
	...
	existing, err := batchRepo.ExistingIdempotencyKeys(ctx, keys)
	if err != nil {
